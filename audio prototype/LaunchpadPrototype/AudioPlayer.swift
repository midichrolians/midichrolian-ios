...
struct AudioManager {
    ...
    init () {
        for i in 0 ... sound.count - 1 {
            for j in 0 ... sound[i].count - 1 {
                if let soundURL = Bundle.main.url(forResource: sound[i][j], withExtension: "wav") {
                    ...
                    { (soundID, _) -> Void in
                        ...
                        nc.post(name:Notification.Name(rawValue:"Sound"),
                            object: nil,
    ...
    func isValidSound(_ indexPath: IndexPath) -> Bool {
        let row = indexPath.section
        let col = indexPath.item
        if sound.count <= row || sound[0].count <= col {
            return false
        }
